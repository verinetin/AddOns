...
local blacklist = { "TargetFrame", "PetAction", "TotemFrame", "PartyFrame", "TargetofTargetFrame", "FocusFrame", "RaidFrame" }
...
function CooldownCount:OnEnable()
	...
	hooksecurefunc("CooldownFrame_SetTimer",CooldownCount.SetCooldown);
...
--function CooldownCount:SetCooldown(frame, start, duration, enable)
--function CooldownCount.SetCooldown(frame, start, duration, enable)
function CooldownCount.SetCooldown(frame, start, duration, enable, charges, maxCharges)
	--black list handle
	...
	if start and start > 0 and duration > CooldownCount.db.profile.minimumDuration and enable > 0
	   and ((maxCharges == nil or maxCharges == 0) or (charges == nil or charges == 0))
	then
